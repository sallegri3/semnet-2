...
"""
Latent Semantic Relationships (LSRs) are underlying semantics between 
nodes that are not directly apparent in the graph edges. They may be
represented by weighted sets of metaedges spanning multiple nodes.

How to Use
1. Create a set of example pairs that exemplify the relationship of interest
2. Start a Neo4j instance containing the semnet database at `localhost:7474`
3. Generate negative examples based on the pairs
4. Submit queries to the database to compute metapath-based count metrics
5. Compute the latent semantic relationship in terms of important metapaths
6. Train the ranking model
7. Choose the source node of interest and rank potential target nodes

-------
Reference:
Wang, Chenguang, et al. "Relsim: relation similarity search in schema-
rich heterogeneous information networks." Proceedings of the 2016 SIAM 
...
def compute_mp_weights(mp_count_mat, c):
	""" Computes the optimal LSR weights by solving the linear 
	programming problem formulated by Wang et al. 2016 """
